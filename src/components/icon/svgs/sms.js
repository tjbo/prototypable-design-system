import { Icon } from '@chakra-ui/react'

export default function (props) {
  return (
    <Icon viewBox="0 0 501.551 501.551" {...props}>
      <path
        d="M309.29 0H20.898C9.404 0 0 9.404 0 20.898v459.755c0 11.494 9.404 20.898 20.898 20.898H309.29c11.494 0 20.898-9.404 20.898-20.898V20.898C330.188 9.404 320.784 0 309.29 0z"
        fill="#40596b"
      />
      <path fill="#84dbff" d="M26.676 59.653h277.943v364.669H26.676z" />
      <g fill="#334a5e">
        <path d="M188.082 450.351h-51.2c-6.269 0-11.494 5.224-11.494 11.494s5.224 11.494 11.494 11.494h51.2c6.269 0 11.494-5.224 11.494-11.494s-5.225-11.494-11.494-11.494zM206.89 25.078h-83.592c-3.135 0-5.224 2.09-5.224 5.224 0 3.135 2.09 5.224 5.224 5.224h83.592c3.135 0 5.224-2.09 5.224-5.224 0-3.135-2.09-5.224-5.224-5.224z" />
      </g>
      <path
        d="M479.608 119.118H224.653c-12.539 0-21.943 10.449-21.943 21.943v146.286c0 12.539 10.449 21.943 21.943 21.943h8.359v54.335l47.02-54.335h199.576c12.539 0 21.943-10.449 21.943-21.943V141.061c0-12.539-10.449-21.943-21.943-21.943z"
        fill="#f2f2f2"
      />
      <g fill="#54c0eb">
        <circle cx="276.898" cy="214.204" r="24.033" />
        <circle cx="352.131" cy="214.204" r="24.033" />
        <circle cx="426.318" cy="214.204" r="24.033" />
      </g>
    </Icon>
  )
}
